...
class BeliefPropagation_int(BeliefPropagation):
    ...
    def query_ev_sensit(
            ...
            common_vars = set(evidence[0] if evidence is not None else []).intersection(
                set(variables)
            )
            if common_vars:
                raise ValueError(
            ...
            for i in range(len(evidence)):
                ...
                if isinstance(self.model, BayesianNetwork_int):
                    self.model, evidence[i] = self._prune_bayesian_model(variables, evidence[i])
            ...
            with open("Comparison_evidence.txt", "w") as o:
                for i,valor in enumerate(results): 
                    for (clave, value) in evidence[i].items():
                        o.write(str(clave)+' = '+str(value))
                        o.write('\n')
                    o.write(str(valor))
...
alarm_model = BayesianNetwork_int(
    [
        ("Burglary", "Alarm"),
        ("Earthquake", "Alarm"),
        ("Alarm", "JohnCalls"),
...
inference2=BeliefPropagation_int(alarm_model)

# inference2.query_ev_sensit(variables=['Alarm'], evidence=[{'MaryCalls':1}, {'Burglary':0}])
inference2.query_ev_sensit(variables=['MaryCalls'], evidence=[{'JohnCalls':1}])

# print(inference2.map_query(variables=['MaryCalls'], evidence={'Burglary':1} ))
# print(inference2.map_query(variables=['MaryCalls'], evidence={'Burglary':0} ))
# print(inference2.map_query(variables=['MaryCalls'], evidence={'Alarm':1} ))
#inference2.query_int(variables=['MaryCalls'], evidence={'Burglary':1})
